...
const whitColor = Colors.white;
const blacColor = Color(0xff222222);
const blac2Color = Color(0xff434242);
const mColor = Color(0xff181D31);
const m3Color = Color(0xff678983);
const m4Color = Color(0xffF0E9D2);
const m5Color = Color(0xff22A39F);
const m6Color = Color(0xffF3EFE0);
//

const kDefaultPadding = 20.0;
/////////////////////////////
double quranFontSize = 22;
//double moshafFontSize = 40;
const String Basmala = 'بِسْمِ اللَّـهِ الرَّحْمَـٰنِ الرَّحِيمِ';

Future setFontSize() async {
  final sharedpref = await SharedPreferences.getInstance();
  await sharedpref.setInt('quranFontSize', quranFontSize.toInt());
}

Future getFontSize() async {
  try {
    final sharedpref = await SharedPreferences.getInstance();
    quranFontSize = sharedpref.getInt('quranFontSize')!.toDouble();
    //   moshafFontSize = sharedpref.getInt('moshafFontSize')!.toDouble();
  } catch (_) {
    quranFontSize = 28;
    //   moshafFontSize = 40;
  }
...
Future setViewMode() async {
  final sharedpref = await SharedPreferences.getInstance();
  await sharedpref.setBool('isView', isView);
}

Future getViewMode() async {
  try {
    final sharedpref = await SharedPreferences.getInstance();
    isView = sharedpref.getBool('isView')!;
  } catch (_) {
    isView = false;
  }
}
